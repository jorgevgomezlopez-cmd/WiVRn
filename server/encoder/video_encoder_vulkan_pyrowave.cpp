...
namespace wivrn
{
    ...
    video_encoder_vulkan_pyrowave::video_encoder_vulkan_pyrowave(
        ...
        uint8_t stream_idx)
    : video_encoder(vk, stream_idx, vk.queue.family_index, settings, nullptr, false),
    vk(vk)
    {
    ...
    void video_encoder_vulkan_pyrowave::init_compute_pipeline()
    {
        try
        {
            auto shader_module = vk.load_shader("pyrowave_encode.comp");
        ...
        uint64_t frame_index)
    {
        try
        {
            // 1. Reservar buffer en CPU con un shared_ptr y delega la liberación
            auto cpu_buffer = std::make_shared<std::vector<uint8_t>>(output_buffer_size);

            // 2. Mapear la memoria de la GPU y copiar los datos al buffer reservado
            ...
            // 3. Crear el struct data de WiVRn
            ...
            encoded_frame.mem = cpu_buffer; // Mantiene vivo cpu_buffer hasta que WiVRn lo envíe
        ...
        {
            std::cerr << "[Pyrowave] Error leyendo frame desde GPU: " << e.what() << std::endl;
            return std::nullopt;
        }
    }
} // namespace wivrn
